classdef PlateLoaderFrontend < hgsetget
    ...
    methods
        function obj = PlateLoaderFrontend(portNumber)
            fprintf('Connecting to server robot...'); 
            obj.xAxisPosition = 3;
        ...
        function response = sendCommand(obj,command)
            ...
            uri = URI("http://snydergipi@snydergi-pi400.rose-hulman.edu:5000/api/" + command);            
            resp = send(r,uri);
...
%             writeline(obj.serialRobot,'Z-AXIS RETRACT');
            obj.isZAxisExtended = false;
            retCommand = sprintf('Z-AXIS RETRACT');
            response = obj.sendCommand(retCommand);
%             response = readline(obj.serialRobot);
        end
...
%             writeline(obj.serialRobot,moveCommand);
            response = obj.sendCommand(moveCommand);
%             response = readline(obj.serialRobot);
            if( contains(response,'ERROR'))
